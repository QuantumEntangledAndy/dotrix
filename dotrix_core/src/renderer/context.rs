...
use super::{Bindings, ComputeArgs, DepthBufferMode, DrawArgs, GpuMesh, PipelineInstance};
...
impl Context {
    ...
    pub(crate) fn run_render_pipeline<Mesh: GpuMesh>(
        ...
        let count: u32 = mesh.get_vertex_count();

        if let Some(index_buffer) = mesh.get_indicies().as_ref() {
